...
app.post("/register", async (req, res) => {
	...
	if (password.length < 8) {
		return res
			.status(400)
			.json({
				message: "Your password length should be 8 or more characters long!",
			});
	}
...
async function updateUserStatus(email, status) {
	try {
		...
		return JSON.stringify({ message: "Payment successfully" });
	} catch (error) {
		console.error("Error updating user record:", error);
...
app.post("/stripe-webhook", async (req, res) => {
	...
	const eventHandlers = {
		"payment_intent.succeeded": async () => {
			...
			const status = "paid";
			try {
				const resp = await updateUserStatus(email, status);
